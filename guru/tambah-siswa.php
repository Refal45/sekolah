...
$stmtKelas = $pdo->query("SELECT id_kelas, nama_kelas, tingkat FROM kelas ORDER BY nama_kelas ASC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validasi
    if (empty($nama_siswa) || empty($nis) || empty($username_input) || empty($password) || empty($id_kelas) || empty($jenis_kelamin)) {
        $error = 'Semua field wajib diisi, kecuali No HP!';
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT id_siswa FROM siswa WHERE nis = ? OR username = ?");
            $stmt->execute([$nis, $username_input]);
            ...
            if ($stmt->fetch()) {
                ...
                // Insert siswa baru
                $stmt = $pdo->prepare("INSERT INTO siswa (nama_siswa, nis, username, password, id_kelas, no_hp, jenis_kelamin) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$nama_siswa, $nis, $username_input, $hashed_password, $id_kelas, $no_hp ?: null, $jenis_kelamin]);
                ...
                <header class="main-header desktop-header flex justify-between items-center">
                    <div>
                        <h1 class="text-xl font-bold flex items-center gap-2">
                            <i class="fa-solid fa-user-plus text-green-600"></i> Tambah Siswa Baru
                        </h1>
                        <p class="text-slate-500 mt-1">Formulir untuk menambahkan data siswa baru ke sistem</p>
                    </div>
            ...
            <div class="content-wrapper content-spacing">
                <main class="page-content overflow-y-auto">
                    ...
                    <div class="flex items-center gap-2 text-sm text-slate-600 mb-6">
                        ...
                        <span class="text-blue-600 font-medium">Tambah Siswa</span>
                    </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="form-help">Nomor Induk Siswa harus unik</div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        <label for="id_kelas" class="form-label">
                                            <i class="fa-solid fa-door-open mr-2"></i>Kelas & Bidang Keahlian *
                                        </label>
                                        <select id="id_kelas" name="id_kelas" required
                                            class="form-input <?= $error && empty($_POST['id_kelas']) ? 'error' : '' ?>">
                                            <option value="">-- Pilih Kelas & Bidang Keahlian --</option>
                                            <?php foreach ($kelasList as $kelas): ?>
                                                <option value="<?= $kelas['id_kelas']; ?>" 
                                                    <?= (isset($_POST['id_kelas']) && $_POST['id_kelas'] == $kelas['id_kelas']) ? 'selected' : '' ?>>
                                                    <?= htmlspecialchars($kelas['tingkat'] . ' - ' . $kelas['nama_kelas']); ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                    ...
                    <div class="card p-6 mt-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-slate-700">
                            <div class="flex items-start gap-3">
                                <i class="fa-solid fa-user-graduate text-green-600 mt-1"></i>
                                <div>
                                    <strong>Data Siswa</strong>
                                    <p class="text-slate-600 mt-1">Pastikan data siswa yang dimasukkan sudah benar dan lengkap</p>
                                </div>
                            </div>
                            <div class="flex items-start gap-3">
