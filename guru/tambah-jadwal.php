...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Validasi id_petugas ada di database
        $stmt_check = $pdo->prepare("SELECT id_petugas FROM petugas WHERE id_petugas = ?");
        $stmt_check->execute([$id_petugas]);
        if (!$stmt_check->fetch()) {
        ...
        $result = $stmt->execute([$id_kelas, $id_mapel, $id_petugas, $hari, $jam_mulai, $jam_selesai]);
        ...
        if ($result) {
            ...
            throw new Exception("Gagal menyimpan data jadwal!");
        }
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
